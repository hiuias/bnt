...
**BNT** 是一款高性能、高安全的 **二进制加密令牌** Go 库，替代传统 令牌。基于 **AES\-256\-GCM 加密 \+ HMAC\-SHA256 完整性校验**，无多余 JSON 冗余，性能优秀，内置密钥轮换、令牌续签、全套安全校验机制。

## 核心特性

- **极致性能**：二进制结构，签名/验签性能优秀

- **银行级安全**：AES\-256\-GCM 对称加密 \+ HMAC\-SHA256 防篡改双校验

- **密钥轮换** ：内置 KID 密钥 ID 机制，支持平滑密钥升级、多密钥并存

- **令牌续签**：支持可控次数刷新令牌，自定义最大续签次数，防无限续期

- **全维度校验**：过期时间、生效时间、签发时间、令牌ID、头部合法性、密钥匹配校验

- **高鲁棒性**：完整 Base64 校验、参数边界限制、防畸形输入攻击

- **完善错误体系**：分层错误定义，支持`errors.Is` 精准捕获错误类型

- **安全防护**：Token 最大 8KB 限制、随机数防重放、头部版本/保留位/标志位强校验

## 安装

```Plain Text
go get github.com/hiuias/bnt
```
...
所有错误均支持 `errors.Is` 解析，禁止直接 `==` 判断（内部错误包装）：

```go
// 正确写法
if errors.Is(err, bnt.ErrTokenExpired) {
	// 处理过期逻辑
}
```
...
测试环境：`Windows i7-8650U 1.90GHz`

```bash
BenchmarkSign‑8                   243897     5230 ns/op   2930 B/op  23 allocs/op
BenchmarkVerify‑8                 281883     4994 ns/op   2136 B/op  22 allocs/op
BenchmarkToken_SignedString‑8     290481     4744 ns/op   2962 B/op  23 allocs/op
BenchmarkParse_FullEndToEnd‑8     235556     4853 ns/op   2160 B/op  22 allocs/op
```

- **签名 QPS：约 210000 **
- **验签 QPS：约 206000**
- **完整签发 (SignedString)：4.74μs，约 21 万 QPS / 核**
- **完整验签 (Parse)：4.85μs，约 20.6 万 QPS / 核**
- 性能优越，无 JSON 序列化开销，二进制解析极速
...
4. **精简Claims**：仅存放核心标识（jti、时间），禁止存放大量业务数据，避免Token膨胀

5. **强制UTC时间**：所有时间参数必须使用 UTC，杜绝时区校验错误

6. **限制续签次数**：生产环境务必配置 `MaxIssueCount`，禁止无限续期
